...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const populateOpt = populate || Alumno.associations
  ...
  add: async (values) => {
    ...
    return Alumno.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    ...
    const entry = await Alumno.updateOne(params, data, { multi: true });
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Alumno.attributes).reduce((acc, curr) => {
      switch (Alumno.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
